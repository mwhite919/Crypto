...
export default function Page({ params }: { params: { id: string } }) {
  ...
  const ath = coinInfo.market_data?.ath.{currency};
  const athChange = coinInfo.market_data?.ath_change_percentage?.{currency};
  const athDate = coinInfo.market_data?.ath_data?.{currency};
  const atl = coinInfo.market_data?.atl?.{currency}; 
  const atlChange = coinInfo.market_data?.atl_change_percentage?.{currency};
  const atlDate = coinInfo.market_data?.atl_data?.{currency};
  const marketCap = coinInfo.market_data?.market_cap.{currency};
  const fullyDiluted = coinInfo.market_data?.fully_diluted_valuation.{currency};
  const volume24h = coinInfo.market_data?.price_change_24h;
  const volumeMarket = volume24h / marketCap;
  const totalVolume = coinInfo.market_data?.total_volume.{currency};
